...
export function StrategyForm({ types, strategy }: StrategyFormProps) {
  ...
  function buildPayload(): StrategyCreate | null {
    ...
    if (sl === null || sl <= 0 || sl > 100) e.stop_loss_pct = "Enter a percent between 0 and 100";
    ...
    setErrors(e);
    if (Object.keys(e).length > 0 || !form.strategy_type || capital === null || risk === null || sl === null) {
      return null;
    }
    return {
      name: form.name.trim(),
          ...
          {(typeInfo?.parameters ?? []).map((p) => (
            ...
            >
              <Input
                ...
                onChange={(e) =>
                  setForm((f) => ({ ...f, parameters: { ...f.parameters, [p.key]: e.target.value } }))
                }
        ...
        <FormSection title="Capital & risk">
          ...
          <div className="flex items-center gap-2.5 sm:col-span-2">
            <Switch
              ...
              onCheckedChange={(c) => set("allow_short", c)}
            />
        ...
        <FormSection title="Execution">
          ...
          {form.trading_mode === "LIVE" ? (
            <p className="flex gap-2 rounded-md border border-loss/40 bg-loss/10 p-3 text-sm text-loss sm:col-span-2">
              <AlertTriangle className="mt-0.5 size-4 shrink-0" />
              <span>
                <strong>LIVE mode trades real money.</strong> Orders go to the real exchange through your
                broker. Backtest and paper trade this strategy first. Live orders are only accepted when
                every backend live-trading guard passes (see the Brokers page).
              </span>
            </p>
          ) : null}
        </FormSection>
